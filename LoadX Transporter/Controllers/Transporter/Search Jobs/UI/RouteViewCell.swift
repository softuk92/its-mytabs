...
open class RouteViewCell: UITableViewCell, NibReusable {
    ...
    var acceptRouteAct: ((RouteViewCell) -> Void)?
    ...
    open func bindLabels(route: Routes) {
        ...
        pickup.text = getAddress(street: route.pu_street, route: route.pu_route, city: route.pu_city, postcode: route.pu_post_code, town: route.pu_town)
        dropoff.text = getAddress(street: route.do_street, route: route.do_route, city: route.do_city, postcode: route.do_post_code, town: route.do_town)
...
//        acceptRoute.roundCorners(corners: [.bottomRight], radius: 5)
    ...
    @IBAction func acceptRouteBtn(_ sender: Any) {
        acceptRouteAct?(self)
    }
